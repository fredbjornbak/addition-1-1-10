...
const AdditionWorkspace: React.FC<AdditionWorkspaceProps> = ({
  ...
  if (!problem) {
    return <Card className="p-3 shadow-grade-card bg-white rounded-grade-card border-0" style={{
    ...
    borderBottom: '10px solid #2F2E41'
      ...
      <div className="flex flex-col md:flex-row items-center gap-3">
        ...
        <div className="flex-1">
          <WorkspaceSection title={`First: ${problem.num1}`} workspaceId="first" tensCount={firstNumberTens} onesCount={firstNumberOnes} onBlocksChange={onFirstNumberChange} resetTrigger={resetTrigger} backgroundColor="rgba(0, 123, 255, 0.1)" borderColor="#007BFF" canReceiveFromOthers={false} />
        </div>
        ...
        {/* Second Number */}
        <div className="flex-1">
          <WorkspaceSection title={`Second: ${problem.num2}`} workspaceId="second" tensCount={secondNumberTens} onesCount={secondNumberOnes} onBlocksChange={onSecondNumberChange} resetTrigger={resetTrigger} backgroundColor="rgba(40, 167, 69, 0.1)" borderColor="#28A745" canReceiveFromOthers={false} />
        </div>
      </div>
    </Card>;
};
